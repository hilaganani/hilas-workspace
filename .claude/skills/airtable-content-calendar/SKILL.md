...
| פעולה | מותר | אסור |
|---|---|---|
| **יצירה** (`create-items`) | שורה חדשה בטבלת "תכנון תוכן חודשי" (`tbl86pJ76WWc3moL2`) בלבד, תמיד עם `סטטוס = מתוכנן` | כל טבלה אחרת, כל סטטוס אחר בזמן היצירה |
...
**חד-פעמי, ידני, באיירטייבל עצמה** — לוודא שבטבלה "תכנון תוכן חודשי" (`tbl86pJ76WWc3moL2`) קיימות העמודות הבאות, בדיוק בשם הזה (רגישות לרווחים/איות):
