...
## דף המכירה ב-noindex עד סמוך להשקה

**`7-marketing-foundations` מסומן `noindex: true`** בטבלת `courses` — וכך גם קורס ה-ManyChat. זו **החלטה מכוונת של הילה**, לא תקלה; היא תפתח את זה יום-יומיים לפני ההשקה (כלומר סביב 17–18.8).

זה **דגל פר-קורס, לא ברמת האתר**. `robots.txt` דווקא מתיר סריקה של דפי הקורסים; החסימה היא תג `<meta name="robots" content="noindex, nofollow">` שנוסף ב-`src/routes/course.$slug.tsx` כשהעמודה דולקת. ארבעת הקורסים בסטטוס `soon` פתוחים לאינדוקס.
