...
- **מירב (Merav)** — visual generator. Takes an image brief (usually relayed by Roi from Noga's `{{IMAGE_NEEDED: ...}}` placeholders) and calls **`gpt-image-2`** (OpenAI Images API) through the `gpt-image-gen` skill to produce content images, saved to `merav/outputs/` alongside a `.txt` of the prompt used. Scans `merav/reference/` for style consistency across images. No web access beyond the API call itself and never dispatches other agents. Persona: [`.claude/agents/merav.md`](.claude/agents/merav.md).
...
For Merav, place 2–6 style reference images (`.png`, `.jpg`, `.jpeg`, `.webp`) in `merav/reference/` — she scans them for style/palette/composition cues before generating each image, for visual consistency across the project.
...
- **Merav agent workflow**: (1) receive an image brief from Roi (usually one of Noga's `{{IMAGE_NEEDED}}` descriptions), (2) scan `merav/reference/` for style/palette/composition cues if it isn't empty, (3) expand the brief into a rich design prompt combining the request with the extracted style, (4) call the `gpt-image-gen` skill (model `gpt-image-2` — never substitute another model; API failures are almost always a bad key or bad parameters, not the model name), (5) save the image to `merav/outputs/<YYYY-MM-DD>-<slug>.png` plus a matching `.txt` of the prompt used, (6) verify the file exists and is larger than 0 bytes before reporting success, (7) report the file path(s) and which references (if any) were used, back to Roi. Merav never calls other agents. Roi is responsible for embedding Merav's images into Noga's `.md`/`.html` files in place of the `{{IMAGE_NEEDED}}` placeholders and saving the final combined version to `output/`.
