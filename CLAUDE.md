...
- **מירב (Merav)** — visual generator. Takes an image brief (usually relayed by Roi from Noga's `{{IMAGE_NEEDED: ...}}` placeholders) and calls **`gpt-image-2`** (OpenAI Images API) through the `gpt-image-gen` skill to produce content images, saved to `merav/outputs/` alongside a `.txt` of the prompt used. Scans `merav/reference/` for style consistency across images, and — for social-media images or explicitly "branded" requests — follows `merav/brand-guidelines.md` (color palette, graphic style, tone). No web access beyond the API call itself and never dispatches other agents. Persona: [`.claude/agents/merav.md`](.claude/agents/merav.md).
